...
protocol NetworkService {
    func dataReceiving<T: Decodable>(
        url: String,
        decodeStruct: T.Type,
...
class NetworkServiceImpl: NetworkService {
    /* Задание функции получения данных.
     Входные значения данной функции - адрес URL, структура-адресат,
     замыкание обработки результата.
     Входное значение замыкания - данные из сети в виде стандартного перечисления Result,
     которые могут быть либо успешно полученными данными, либо ошибкой */
    func dataReceiving<T: Decodable>(
        url: String,
        decodeStruct: T.Type,
        completion: @escaping (Result<T, Error>) -> Void
